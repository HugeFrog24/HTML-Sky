...
A basic game backend needs to call `HTiSetGameStatus()` to set the game status, then call `HTiSetupAll()` to load configuration and the mod. The above functions must be called in sequence within the same thread.
